...
def send_email(to_email, subject, body):
    try:
        ...
        response = ses.send_email(
            Source='nimesatesting@gmail.com',
            Destination={'ToAddresses': [to_email, 'vishal.mokani@jeavio.com']},
            Message={
...
def lambda_handler(event, context):
    try:
        ...
        subject = f"AWS Billing Details - {today.strftime('%Y-%m-%d')} (Total Month Cost: ${current_month_total_cost:.2f})"
        body = (
        ...
        # Send email
        if send_email('nimesatesting@gmail.com', subject, body):
            return {
                'statusCode': 200,
